		...
		<ul class="gallery">
			<?php foreach ($_posts as $key=>$item) { ?>	
				<li style="width:<?php echo round(100/$columns); ?>%" class="<?php echo str_replace(',', ' ', $item->post_excerpt) ?>">
					<?php $att1 = wp_get_attachment_image_src($item->ID, 'large'); ?>
					<a class="hs-rsp-popup <?php echo ($modal=='yes')? 'hiddendiv':'' ?> img-polaroid" href="<?php echo ($modal=='yes')? '#modal-' . $key . '':$att1[0] ?>">
						<?php
