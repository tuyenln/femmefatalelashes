...
class Nova_Widget_Best_Sellers extends WP_Widget {
	 ...
	 * @return void
	 */
	function Nova_Widget_Best_Sellers() {

		/* Widget variable settings. */
		$this->woo_widget_cssclass = 'woocommerce best_sellers';
		$this->woo_widget_description = __( 'Display a list of your best selling products on your site.', 'woocommerce' );
		$this->woo_widget_idbase = 'nova_best_sellers';
		$this->woo_widget_name = __( 'Nova Best Sellers', 'woocommerce' );

		/* Widget settings. */
		$widget_ops = array( 'classname' => $this->woo_widget_cssclass, 'description' => $this->woo_widget_description );

		/* Create the widget. */
		$this->WP_Widget('nova_best_sellers', $this->woo_widget_name, $widget_ops);
	...
	function widget( $args, $instance ) {
		...
		$cache = wp_cache_get('best_sellers', 'widget');
		...
		$title = apply_filters('widget_title', empty($instance['title']) ? __('Best Seller', 'woocommerce' ) : $instance['title'], $instance, $this->id_base);
		...
		if ( $r->have_posts() ) {
			...
			if ( $title )
				echo $before_title . $title ."<i class=\"icon-right-open-big\"></i>". $after_title;
    ...
    jQuery(window).load(function(){
	  ...
	  if(width_doc >960){
	      jQuery(\'.best-sellers\').flexslider({
	        animation: "slide",
	        itemWidth: 212,
	        itemMargin: 18,
	        animationLoop: true,
	  ...
	  if(width_doc <900){
	      jQuery(\'.best-sellers\').flexslider({
			...
			<div class="best-sellers flexslider products-list-home">
			<ul class="slides">';
		...
		wp_cache_set('best_sellers', $cache, 'widget');
	}
	...
	function update( $new_instance, $old_instance ) {
		...
		if ( isset($alloptions['best_sellers']) ) delete_option('best_sellers');
	...
	function flush_widget_cache() {
		wp_cache_delete( 'best_sellers', 'widget' );
	}
