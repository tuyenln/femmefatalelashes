			...
			foreach ($customer_orders as $customer_order) :
				...
				?><tr class="order">
					...
					<td class="order-actions" style="text-align:right; white-space:nowrap;">
						...
						<?php if (in_array($order->status, array('pending', 'failed'))) : ?>
							<a href="<?php echo esc_url( $order->get_checkout_payment_url() ); ?>" class="btn pay"><?php _e('Pay', 'woocommerce'); ?></a>
						<?php endif; ?>

						<a href="<?php echo esc_url( add_query_arg('order', $order->id, get_permalink(woocommerce_get_page_id('view_order'))) ); ?>" class="btn"><?php _e('View', 'woocommerce'); ?></a>
